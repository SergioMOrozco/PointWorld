...
class TeleopPointCloudSystem:
    ...
    def step(
    ...
    ) -> tuple[list[dict], list[dict], o3d.geometry.PointCloud, np.ndarray, dict[str, np.ndarray]]:
        ...
        Returns:
            actions: Per-leader action dicts (joint/gripper targets) used to drive this step.
        ...
        datapoints, scene_pcd, robot_pcd, robot_link_pcds = self.viewer.update(
            *actions, masks_by_serial=masks_by_serial
        )
        return actions, datapoints, scene_pcd, robot_pcd, robot_link_pcds
...
def main() -> None:
    ...
    try:
        while not system.viewer.quit:
            ...
            _actions, _datapoints, _scene_pcd, _robot_pcd, _robot_link_pcds = system.step()
            breakpoint()
